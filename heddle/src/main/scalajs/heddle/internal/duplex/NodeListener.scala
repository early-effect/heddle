...
import heddle.internal.node.{AddressInfo, Net, NetServer, NetSocket}
import scala.collection.mutable
import scala.scalajs.js
import zio.*

private[heddle] final class NodeListener(server: NetServer) extends Listener:
  private val pending                         = mutable.Queue.empty[NetSocket]
  ...
  server.on(
    ...
    (
        (sock: NetSocket) =>
          if waiters.nonEmpty then waiters.dequeue()(ZIO.succeed(NodeConn.of(sock)))
          else pending.enqueue(sock)
    ): js.Function1[NetSocket, Unit],
  )
  server.on(
    "close",
    (() =>
  ...
  def accept: Task[ByteConn] =
    ZIO.suspendSucceed {
      if pending.nonEmpty then ZIO.succeed(NodeConn.of(pending.dequeue()))
      else if closed then ZIO.fail(java.io.IOException("listener closed"))
      else
        ZIO.async[Any, Throwable, ByteConn] { cb =>
          if pending.nonEmpty then cb(ZIO.succeed(NodeConn.of(pending.dequeue())))
  ...
  def bind(config: Server.Config): IO[ServerError, Listener] =
    ZIO.async[Any, ServerError, Listener] { cb =>
      ...
      val _ = server.listen(
        config.port,
        config.host,
        () => cb(ZIO.succeed(NodeListener(server))),
      )
    }
end NodeListener
