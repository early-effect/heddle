...
private[heddle] object ConnBuf:
  def channel(buf: ByteBuffer, ch: SocketChannel): ConnBuf =
    buf.limit(0)
    ConnBuf(buf, b => ZIO.attempt(ch.read(b)).mapError(HttpError.Io(_)))

  def inputStream(buf: ByteBuffer, in: java.io.InputStream): ConnBuf =
    fromPull(buf, Tls.pull(in, math.max(1024, buf.capacity.min(16 * 1024))))

  def fromPull(buf: ByteBuffer, pull: IO[HttpError, Option[Chunk[Byte]]]): ConnBuf =
    buf.limit(0)
