...
private[heddle] object ClientPlatform:
  ...
  private def oneShot(
    ...
    ZIO.scoped {
      for
        ...
        t      <- ZIO.acquireRelease(open(cfg, SSLContext.getDefault, target))(c => ZIO.succeed(c.close()))
  ...
  private def evictLoop(cfg: Client.Config, idle: Ref[Map[PoolKey, PoolState]]): UIO[Nothing] =
    val tick =
  ...
  private def evictExpired(cfg: Client.Config, idle: Ref[Map[PoolKey, PoolState]]): UIO[Unit] =
  ...
  private final class Pooled(cfg: Client.Config, ssl: SSLContext, idle: Ref[Map[PoolKey, PoolState]]) extends Client:
    ...
    private def acquire(key: PoolKey, target: Target): Task[Transport] =
      Clock.nanoTime
        ...
        .flatMap {
          ...
          case Take.OpenNew  =>
            open(cfg, ssl, target).tapError(_ =>
              idle.update { m =>
                val st = m.getOrElse(key, PoolState(Chunk.empty, 0))
  ...
  private def open(cfg: Client.Config, ctx: SSLContext, target: Target): Task[Transport] =
  ...
  def sse(url: String, config: Client.Config): ZStream[Any, Throwable, ServerSentEvent] =
    ZStream.unwrapScoped {
      for
        target <- ZIO.attempt(Target.parse(url))
        t      <- ZIO.acquireRelease(open(config, SSLContext.getDefault, target))(c => ZIO.succeed(c.close()))
